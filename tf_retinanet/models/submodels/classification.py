...
def default_classification_model(
    ...
    Args:
        num_classes:                    Number of classes to predict a score for at each
                                            feature level.
        num_anchors:                    Number of anchors to predict classification scores for at
                                            each feature level.
        pyramid_feature_size:           The number of filters to expect from the feature pyramid
                                            levels.
        prior_probability:              Probability for the bias initializer of the last
                                            convolutional layer.
        classification_feature_size:    The number of filters to use in the layers in the
    ...
    if tf.keras.backend.image_data_format() == 'channels_first':
        outputs = tf.keras.layers.Permute(
            (2, 3, 1),
            name='pyramid_classification_permute'
            )(outputs)
...
class ClassificationSubmodel(Submodel):
    ...
    def __init__(self, config, **kwargs):
        ...
        if config['num_classes'] < 1:
            raise ValueError(
                "Please indicate a positive number of classes for classification submodel.")
