...
def assert_training_model(model):
    ...
    """
    assert all(
        output in model.output_names for output in ['bbox_regression', 'classification']), \
        "Input is not a training model (no 'bbox_regression' and 'classification' outputs"\
        " were found, outputs are: {}).".format(model.output_names)
...
def retinanet(
    ...
    Returns:
        A keras.models.Model which takes an image as input and outputs generated anchors and the
            result from each submodel on every pyramid level.
    ...
    for submodel in submodels:
        retinanet_submodels.append(
            (
                submodel.get_name(),
                submodel.create(
                    num_anchors=num_anchors,
                    name='{}_submodel'.format(
                        submodel.get_name()
                        )
                    )
                )
...
def retinanet_bbox(
    ...
    """
    Construct a RetinaNet model on top of a backbone and adds convenience functions to output
        boxes directly.

    This model uses the minimum retinanet model and appends a few layers to compute boxes within
        the graph.
    These layers include applying the regression values to the anchors and performing NMS.

    Args:
        model:                  RetinaNet model to append bbox layers to. If None, it will create
                                    a RetinaNet model using **kwargs.
        nms:                    Whether to use non-maximum suppression for the filtering step.
        class_specific_filter:  Whether to use class specific filtering or filter for the best
                                    scoring class only.
        name:                   Name of the model.
