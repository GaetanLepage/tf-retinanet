...
def anchor_targets_bbox(
    ...
    Args:
        ...
        annotations_group:  List of annotations (np.array of shape (N, 5)
                                for (x1, y1, x2, y2, label)).
        num_classes:        Number of classes to predict.
        mask_shape:         If the image is padded with zeros, mask_shape can be used to mark the
        ...
        positive_overlap:   IoU overlap or positive anchors (all anchors
                                with overlap > positive_overlap are positive).

    Returns:
        labels_batch:       batch that contains labels & anchor states (np.array of shape
                                (batch_size, N, num_classes + 1), where N is the number of
                                anchors for an image and the last column defines the anchor state
                                (-1 for ignore, 0 for bg, 1 for fg).
        regression_batch:   batch that contains bounding-box regression targets for an
                                image & anchor states (np.array of shape (batch_size, N, 4 + 1),
                                where N is the number of anchors for an image, the first 4 columns
                                define regression targets for (x1, y1, x2, y2) and the last column
                                defines anchor states (-1 for ignore, 0 for bg, 1 for fg).
    """
    ...
    regression_batch = np.zeros(
        shape=(batch_size, anchors.shape[0], 4 + 1),
        dtype=tf.keras.backend.floatx())
    labels_batch = np.zeros(
        shape=(batch_size, anchors.shape[0], num_classes + 1),
        dtype=tf.keras.backend.floatx())
    ...
    for index, (image, annotations) in enumerate(zip(image_group, annotations_group)):
        if annotations['bboxes'].shape[0]:
            ...
            regression_batch[index, :, :-1] = bbox_transform(
                anchors=anchors,
                gt_boxes=annotations['bboxes'][argmax_overlaps_inds, :])
...
def compute_gt_annotations(
    ...
    overlaps = compute_overlap(
        ...
        b=annotations.astype(np.float64))
...
def layer_shapes(image_shape, model):
    ...
    Args:
        ...
        model:          The model to use for computing how the image shape is transformed in
                            the pyramid.
...
def anchors_for_shape(
    ...
    Args:
        ...
        shapes_callback:    Function to call for getting the shape of the image at different
                                pyramid levels.
