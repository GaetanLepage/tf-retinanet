...
def focal(alpha=0.25, gamma=2.0):
    ...
    def _focal(y_true, y_pred, **kwargs):
        ...
        alpha_factor = tf.where(
            condition=tf.keras.backend.equal(labels, 1),
            x=alpha_factor,
            y=1 - alpha_factor)
        ...
        normalizer = tf.keras.backend.cast(
            x=tf.keras.backend.shape(normalizer)[0],
            dtype=tf.keras.backend.floatx())
        normalizer = tf.keras.backend.maximum(
            tf.keras.backend.cast_to_floatx(1.0),
            normalizer)
...
def smooth_l1(sigma=3.0):
    ...
    def _smooth_l1(y_true, y_pred, **kwargs):
        ...
        regression_loss = tf.where(
            condition=tf.keras.backend.less(regression_diff, 1.0 / sigma_squared),
            x=0.5 * sigma_squared * tf.keras.backend.pow(regression_diff, 2),
