...
class Generator(tf.keras.utils.Sequence):
    ...
    def __init__(
        ...
        Args:
            transform_generator:     A generator used to randomly geometrically transform images
                                         and annotations.
            visual_effect_generator: A generator used to randomly visually transform images and
                                         annotations.
            batch_size:              The size of the batches to generate.
            group_method:            Determines how images are grouped together (defaults to
                                         'ratio', one of ('none', 'random', 'ratio')).
            shuffle_groups:          If True, shuffles the groups each epoch.
            image_min_side:          After resizing the minimum side of an image is equal to
                                         image_min_side.
            image_max_side:          If after resizing the maximum side is larger than
                                         image_max_side, scales down further so that the max side
                                         is equal to image_max_side.
            transform_parameters:    The transform parameters used for data augmentation.
            compute_anchor_targets:  Function handler for computing the targets of anchors for an
                                         image and its annotations.
            compute_shapes:          Function handler for computing the shapes of the pyramid for
                                         a given input.
    ...
    def load_annotations_group(self, group):
        ...
        for annotations in annotations_group:
            assert(isinstance(annotations, dict)), '\'load_annotations\' should return a list of"\
                    " dictionaries, received: {}'.format(type(annotations))
            assert('labels' in annotations), '\'load_annotations\' should return a list of"\
                    " dictionaries that contain \'labels\' and \'bboxes\'.'
            assert('bboxes' in annotations), '\'load_annotations\' should return a list of"\
                    " dictionaries that contain \'labels\' and \'bboxes\'.'
    ...
    def filter_annotations(self, image_group, annotations_group, group):
        """
        Filter annotations by removing those that are outside of the image bounds or
            whose width/height < 0.
        """
        # Test all annotations.
        for index, (image, annotations) in enumerate(zip(image_group, annotations_group)):
            # Test x2 < x1 | y2 < y1 | x1 < 0 | y1 < 0 | x2 <= 0 | y2 <= 0 | x2 >= image.shape[1]
            # | y2 >= image.shape[0].
            ...
            if len(invalid_indices):
                warnings.warn(
                    'Image with id {} (shape {}) contains the following"\
                            " invalid boxes: {}.'.format(
                                group[index],
                                image.shape,
    ...
    def random_visual_effect_group(self, image_group, annotations_group):
        ...
        for index, image in enumerate(image_group):
            ...
            image, annotations_group[index] = self.random_visual_effect_group_entry(
                image, annotations_group[index]
            )
    ...
    def random_transform_group(self, image_group, annotations_group):
        ...
        for index, image in enumerate(image_group):
            ...
            image, annotations_group[index], _ = self.random_transform_group_entry(
                image, annotations_group[index])
    ...
    def preprocess_group(self, image_group, annotations_group):
        ...
        for index, image in enumerate(image_group):
            ...
            image, annotations_group[index] = self.preprocess_group_entry(
                image, annotations_group[index])
