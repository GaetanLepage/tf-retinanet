...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        action='store_false', dest='resize')
    parser.add_argument(
        '--anchors',
    ...
    parser.add_argument(
        '--display-name',
        help='Display image name on the bottom left corner.',
        action='store_true')
    parser.add_argument(
        '--annotations',
        help='Show annotations on the image. Green annotations have anchors,"\
                " red annotations don\'t and therefore don\'t contribute to training.',
        action='store_true')
    ...
    while True:
        ...
        if len(annotations['labels']) > 0:
            ...
            if generator.transform_generator:
                image, annotations, transform = generator.random_transform_group_entry(
                    image,
                    annotations)
            if generator.visual_effect_generator:
                image, annotations = generator.random_visual_effect_group_entry(
                    image,
                    annotations)
            ...
            positive_indices, _, max_indices = compute_gt_annotations(
                anchors=anchors,
                annotations=annotations['bboxes'])
            ...
            if config['debug']['annotations']:
                ...
                draw_boxes(
                    image=image,
                    boxes=annotations['bboxes'][max_indices[positive_indices], :],
                    color=(0, 255, 0))
            ...
            if config['debug']['display_name']:
                draw_caption(
                    image=image,
                    box=[0, image.shape[0]],
                    caption=os.path.basename(generator.image_path(i)))
